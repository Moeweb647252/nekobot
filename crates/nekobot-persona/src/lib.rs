...
impl PersonaMiddleware {
    ...
    fn conn(&self) -> anyhow::Result<Connection> {
        self.app_db.read().map_err(|e| anyhow::anyhow!("persona lock: {e}"))?
            .clone().ok_or_else(|| anyhow::anyhow!("persona not initialized"))
    }
}

#[async_trait::async_trait]
impl Middleware for PersonaMiddleware {
    fn name(&self) -> &'static str { "persona" }
    ...
    async fn before_chat(
        ...
        if let Ok(conn) = self.conn() {
            if let Some(persona) = nekobot_core::entity::persona::get(&conn, &ctx.agent_name).await? {
    ...
    async fn init(&self, ctx: &Context) -> Result<(), anyhow::Error> {
        ...
        *self.app_db.write().map_err(|e| anyhow::anyhow!("persona lock: {e}"))? = Some(conn);

        let tools: Vec<Arc<dyn Tool>> = vec![
            Arc::new(SetPersonaTool { agent_name: ctx.agent_name.clone(), app_db: self.conn()? }),
            Arc::new(GetPersonaTool { agent_name: ctx.agent_name.clone(), app_db: self.conn()? }),
        ];

        let mut specs = self.tool_specs.write().map_err(|e| anyhow::anyhow!("persona lock: {e}"))?;
        for tool in &tools {
            specs.push(ToolSpec {
...
struct SetPersonaTool { agent_name: String, app_db: Connection }

#[async_trait::async_trait]
impl Tool for SetPersonaTool {
    fn name(&self) -> &str { "set_persona" }
    ...
    async fn call(&self, args: Value) -> ToolResult<Value> {
        ...
        nekobot_core::entity::persona::upsert(&self.app_db, &self.agent_name, persona).await
            .map_err(|e| ToolError::Execution(format!("upsert: {e}")))?;
        Ok(Value::String(format!("Persona set: \"{persona}\"")))
    }
}

struct GetPersonaTool { agent_name: String, app_db: Connection }

#[async_trait::async_trait]
impl Tool for GetPersonaTool {
    fn name(&self) -> &str { "get_persona" }
    fn description(&self) -> &str {
    ...
    async fn call(&self, _args: Value) -> ToolResult<Value> {
        let persona = nekobot_core::entity::persona::get(&self.app_db, &self.agent_name).await
